# Finnish Vocabulary

## Kappale 1

| Finnish | English |
|---|---|
| Hei! | Hi! / Hello! |
| ja | and |
| Tervetuloa! | Welcome! |
| Anteeksi! | Excuse me! / Sorry! |
| onko | is there / is (question) |
| täällä | here |
| suomen kurssi | Finnish course |
| joo | yes (informal) |
| olla | to be |
| tämä | this |
| paikka | place / seat |
| vapaa | free / available |
| kiva | nice / fun |
| minä | I |
| kuka | who |
| sinä | you |
| Hauska tutustua! | Nice to meet you! |
| Kiitos samoin! | Thank you, likewise! |
| Moi! | Hi! / Bye! (informal) |
| Mitä kuuluu? | How are you? / What's up? |
| Terve! | Hi! / Hello! (informal) |
| Kiitos! | Thank you! |
| Minulle kuuluu hyvää. | I'm doing well. |
| Entä sinulle? | And you? / What about you? |
| Ihan hyvää, kiitos. | Just fine, thanks. |
| tässä | here (nearby) |
| minun | my |
| nimi | name |
| opettaja | teacher |
| Miten se kirjoitetaan? | How do you spell/write it? |
| etunimi | first name |
| sukunimi | last name / surname |
| okei | okay |
| kurssipäivä | course day |
| kello | clock / o'clock |

---

## Kappale 2

| Finnish | English |
|---|---|
| minkämaalainen | of what nationality / from which country |
| loppua | to end / to run out |
| muu | other |
| opiskelija | student |
| mennä | to go |
| koti | home |
| mutta | but |
| kioski | kiosk / newsstand |
| ostaa | to buy |
| jäätelö | ice cream |
| maksaa | to cost / to pay |
| euro | euro |
| ei | no / not |
| koska | because |
| säästää | to save (money) |
| raha | money |
| sitten | then |
| istua | to sit |
| puisto | park |
| puhua | to speak / to talk |
| nauraa | to laugh |
| brasilialainen | Brazilian |
| venäläinen | Russian |
| mistä | from where |
| kotoisin | originally from |
| Etelä-Afrikka | South Africa |
| Ai jaa! | Oh really! / Is that so! |
| kieli | language |
| äidinkieli | mother tongue / native language |
| englanti | English |
| myös | also / too |
| vähän | a little / a bit |
| espanja | Spanish |
| en puhu | I don't speak |
| Brasilia | Brazil |
| portugali | Portuguese |
| Ai niin! | Oh right! / I see! |
| totta | true / really |
| ajaa | to drive |
| auto | car |
| bussipysäkki | bus stop |
| missä | where |
| asua | to live |
| Pohjois-Helsinki | North Helsinki |
| bussi | bus |
| ei ... vaan | not ... but |
| ensin | first |
| metro | metro / subway |
| nyt | now |
| tulla | to come |

---

## Kappale 3

| Finnish | English |
|---|---|
| soittaa | to call / to play (instrument) |
| ystävä | friend |
| videopuhelu | video call |
| olla töissä | to be at work |
| jo | already |
| ei vielä | not yet |
| vasta | only / just |
| puoli | half |
| Ai niin, totta. | Oh right, true. |
| sää | weather |
| aika | quite / time |
| huono | bad |
| -kin = myös | -kin suffix = also / too |
| Sataa kaatamalla. | It's pouring with rain. |
| On tuulista. | It's windy. |
| Aijaa! | Oh! / Wow! |
| Aurinko paistaa. | The sun is shining. |
| tosi | really / very |
| lämmin | warm |
| aste | degree |
| yö | night |
| niin | so / that way |
| valoisa | bright / light |
| että | that (conjunction) |
| vaikea | difficult |
| nukkua | to sleep |
| nauttia | to enjoy |
| pimeä | dark |
| No joo, niin kulemma! | Well yes, apparently so! |
| onko sinulla | do you have |
| työpaikka | workplace / job |
| etsiä töitä | to look for work |
| helppo | easy |
| Hieno juttu! | That's great! |
| millainen | what kind of |
| mukava | nice / pleasant / comfortable |
| sama | same |
| maanantaisin | on Mondays |
| keskiviikkoisin | on Wednesdays |
| torstaisin | on Thursdays |
| kerta | time / occasion |
| Ohhoh! | Oh! / Wow! |
| yhtä hyvin kuin | as well as |
| Niin varmaan. | I suppose so. / Probably. |
| vielä | still / yet |
| opiskelija-asunto | student apartment |
| muuttaa | to move |
| meidän | our |
| uusi | new |
| Kuulostaa hyvältä. | That sounds good. |
| minun täytyy | I have to / I must |
| lähteä töihin | to leave for work |
| Terveisiä Hannalle! | Greetings to Hanna! |
| kertoa | to tell |

---

## Kappale 4

| Finnish | English |
|---|---|
| naimisissa | married |
| Leikin koiran kanssa | I play with the dog |
| Rakastan sinua | I love you |
| meillä on | we have |
| lapsi | child |
| söpö | cute |
| 4-vuotias | 4 years old |
| tyttö | girl |
| bussikuski | bus driver |
| minulla ei ole | I don't have |
| He etsivät autoa | They look for a car |
| hoikka | slim / slender |
| Pidän kahvista | I like coffee |
| urheilu | sports / exercise |
| rauhallinen | calm / peaceful |
| ehkä | maybe / perhaps |
| ujo | shy |
| eronnut | divorced |
| sinkku | single |
| perhe | family |
| suku | family / relatives / clan |
| pikkusisko | little sister |
| isoveli | big brother |
| täti | aunt |
| setä | uncle |
| serkku | cousin |
| helsinkiläinen | person from Helsinki |
| talo | house / building |
| kerros | floor / storey |
| huone | room |
| keittiö | kitchen |
| puhelias | talkative |
| iloinen | happy / cheerful |
| musta | black |
| kihara | curly |
| tumma | dark (colour) |
| tyttöystävä | girlfriend |
| kauppa | shop / store |
| meillä ei ole | we don't have |
| pyörä | bike / bicycle |

---

## Kappale 5

| Finnish | English |
|---|---|
| herätyskello | alarm clock |
| soida | to ring / to sound |
| käydä suihkussa | to take a shower |
| pukea päälle | to get dressed |
| sen jälkeen | after that |
| aamupala | breakfast |
| lehti | newspaper / magazine / leaf |
| yhdessä | together |
| yleensä | usually / generally |
| muroja | cereal (flakes) |
| leipä | bread |
| ei mitään | nothing |
| vain | only / just |
| kahvi | coffee |
| lounasravintola | lunch restaurant |
| kokki | cook / chef |
| tehdä ruokaa | to cook (food) |
| työkaveri | work colleague |
| päiväkoti | daycare / kindergarten |
| apteekki | pharmacy |
| koulu | school |
| kahdelta = kello kaksi | at two (o'clock) |
| kaveri | friend / buddy |
| jalkapallo | football / soccer |
| iltaisin | in the evenings |
| jotain | something |
| uimahalli | indoor swimming pool |
| tunti | hour / lesson |
| kenkä | shoe |
| valita | to choose / to select |
| sandaalit | sandals |
| väsynyt | tired |
| kirja | book |
| yhdeksältä = kello yhdeksän | at nine (o'clock) |
| pestä hampaat | to brush teeth |
| mennä nukkumaan | to go to sleep |

---

## Kappale 6

| Finnish | English |
|---|---|
| tie | road |
| kerrostalo | apartment building |
| kellari | cellar / basement |
| piha | yard |
| katu | street |
| porras | stair / step |
| rappu | staircase / stairwell |
| lastenhuone | children's room |
| makuuhuone | bedroom |
| olohuone | living room |
| parveke | balcony |
| seinä | wall |
| lattia | floor |
| kirjoituspöytä | desk / writing table |
| työtuoli | office chair |
| yöpöytä | nightstand / bedside table |
| sohvapöytä | coffee table |
| kirjahylly | bookshelf |
| valokuva | photograph |
| laatikko | box / drawer |
| lelu | toy |
| lelulaatikko | toy box |
| korkea | high / tall |
| matala | low |
| leveä | wide |
| suuri | large / big |
| pehmeä | soft |
| naapuri | neighbour |
| asukas | resident / occupant |
| huoltomies | maintenance man |
| huoltoyhtiö | maintenance company |
| ongelma | problem |
| vesihana | tap / faucet |
| rikki | broken |
| vika | fault / defect |
| vuotaa | to leak |
| kunnolla | properly |
| kiinni | closed / shut |
| auki | open |
| sopia | to fit / to suit / to agree |
| kantaa | to carry |
| ulos | out / outside |
| sisään | in / inside |
| ylös | up |
| nainen | woman |
| lounas | lunch |
| pakettiauto | delivery van |
| selvä juttu | all clear / got it |
| koko ajan | all the time |
| tuo | that |
| se | it / that |
| nämä | these |

---

## Seasons

| Finnish | English |
|---|---|
| kevät | spring |
| kesä | summer |
| syksy | autumn / fall |
| talvi | winter |

---

## Months

| Finnish | English |
|---|---|
| tammikuu | January |
| helmikuu | February |
| maaliskuu | March |
| huhtikuu | April |
| toukokuu | May |
| kesäkuu | June |
| heinäkuu | July |
| elokuu | August |
| syyskuu | September |
| lokakuu | October |
| marraskuu | November |
| joulukuu | December |

---

## Days of the Week

| Finnish | English |
|---|---|
| maanantai | Monday |
| tiistai | Tuesday |
| keskiviikko | Wednesday |
| torstai | Thursday |
| perjantai | Friday |
| lauantai | Saturday |
| sunnuntai | Sunday |

---

## Top 100 Finnish Verbs

| Finnish | English |
|---|---|
| voida | to be able to / can |
| saada | to get / to receive |
| tietää | to know (a fact) |
| nähdä | to see |
| antaa | to give |
| ottaa | to take |
| ajatella | to think |
| lähteä | to leave |
| kysyä | to ask |
| katsoa | to watch / to look |
| tarvita | to need |
| haluta | to want |
| pitää | to like / to hold |
| tykätä | to like (informal) |
| rakastaa | to love |
| vihata | to hate |
| osata | to know how to |
| oppia | to learn |
| opiskella | to study |
| lukea | to read |
| kirjoittaa | to write |
| kuunnella | to listen |
| kuulla | to hear |
| ymmärtää | to understand |
| muistaa | to remember |
| unohtaa | to forget |
| vastata | to answer |
| pyytää | to ask for / to request |
| keskustella | to discuss / to chat |
| syödä | to eat |
| juoda | to drink |
| herätä | to wake up |
| nousta | to get up / to rise |
| seisoa | to stand |
| maata | to lie down |
| kävellä | to walk |
| juosta | to run |
| uida | to swim |
| pelata | to play (sports / games) |
| leikkiä | to play (as a child) |
| matkustaa | to travel |
| lentää | to fly |
| saapua | to arrive |
| jäädä | to stay |
| käydä | to visit / to go to |
| tavata | to meet |
| tutustua | to get to know |
| myydä | to sell |
| tilata | to order |
| varata | to reserve / to book |
| laittaa | to put / to place |
| tuoda | to bring |
| viedä | to take (somewhere) |
| avata | to open |
| sulkea | to close |
| löytää | to find |
| etsiä | to search / to look for |
| hoitaa | to take care of |
| auttaa | to help |
| korjata | to fix / to repair |
| siivota | to clean |
| pestä | to wash |
| laulaa | to sing |
| piirtää | to draw |
| maalata | to paint |
| itkeä | to cry |
| hymyillä | to smile |
| pelätä | to fear |
| tuntea | to feel / to know (a person) |
| elää | to live / to be alive |
| syntyä | to be born |
| kuolla | to die |
| laskea | to count / to calculate |
| nostaa | to lift |
| heittää | to throw |
| lainata | to borrow / to lend |
| yrittää | to try |
| onnistua | to succeed |
| käyttää | to use |
| päättää | to decide |
| alkaa | to start / to begin |
| tapahtua | to happen |
| riittää | to be enough |
| palata | to return |
| sanoa | to say |
| tehdä | to do / to make |

---

## Pronominit (Pronouns)

| Finnish | English |
|---|---|
| hän | he / she |
| me | we |
| te | you (plural / formal) |
| he | they |
| hänen | his / her |

---

## Yleiset sanat (General vocabulary)

| Finnish | English |
|---|---|
| venyttely | stretching |
| kertaa | times |
| Kuinka...? | How...? |
| Huomiseen! | See you tomorrow! |
| perusmuoto | base form / dictionary form |
| ääntäminen | pronunciation |
| ei haittaa | no problem / it doesn't matter |
| viiva | line / dash |
| selvä | clear / understood |
| jaettuna | divided / shared |
| Milloin...? | When...? |
| muut | others / the rest |
| apua | help! |
| harjoitus | exercise / practice |
| yhteystiedot | contact details |
| kotitehtävät | homework |
| tauko | break / pause |
| lause | sentence |
| aakkoset | alphabet |
| fraasit | phrases |
| vastaus | answer |
| taito | skill |
| virhe | mistake |
| laina | loan |
| hiukset | hair |
| kepit | sticks / canes |
| kylä | village |
| vuosi | year |
| vartalo | body / torso |
| kansalaisuus | nationality |
| pääsiäinen | Easter |
| kurkku kipeä | sore throat |
| kaupunki | city / town |
| päivä | day |
| kappale | chapter / track |
| läppäri | laptop (informal) |
| kuukausi | month |
| laturi | charger |
| meri | sea |
| esittely | introduction / presentation |
| puutarha | garden |
| tukka | hair (head, informal) |
| töissä | at work |
| töitä | work / jobs |
| häntä | tail |
| viihtyisä | cosy / comfortable |
| Pekan kanssa | with Pekka |
| etu | advantage / benefit |
| ladata | to download / to charge |
| kaataa | to pour / to knock over |
| miettiä | to think / to ponder |

---

## Adverbit (Adverbs)

| Finnish | English |
|---|---|
| aina | always |
| usein | often |
| joskus | sometimes |
| harvoin | rarely / seldom |
| ei koskaan | never |
| heti | immediately / right away |
| kohta | soon |
| ennen | before / previously |
| myöhemmin | later |
| ensi | next |
| paljon | a lot / much |
| tarpeeksi | enough |
| melkein | almost / nearly |
| liian | too / too much |
| todella | really / truly |
| nopeasti | quickly / fast |
| hitaasti | slowly |
| mitään | anything / nothing |
| eli | that is / i.e. |

---

## Adjektiivit (Adjectives)

| Finnish | English |
|---|---|
| hauska | fun / funny |
| vahva | strong |
| heikko | weak |
| hidas | slow |
| pitkä | tall / long |
| lyhyt | short |
| kallis | expensive |
| halpa | cheap |
| ilmainen | free (no cost) |
| ruma | ugly |
| onnellinen | happy |
| viileä | cool (temperature) |
| hiljainen | quiet / silent |

---

## Suunnat (Directions)

| Finnish | English |
|---|---|
| pohjoinen | north |
| etelä | south |
| itä | east |
| länsi | west |
| keski | middle / centre |
| eteenpäin | forward / ahead |
| takana | behind / at the back |

---

## Konjunktiot (Connectives)

| Finnish | English |
|---|---|
| tai | or (when both options are valid) |
| vai | or (when only one answer is correct) |

---

## Sää – lisää ilmaisuja (More weather expressions)

| Finnish | English |
|---|---|
| tuulinen | windy |
| aurinkoinen | sunny |
| pilvinen | cloudy |
| on sumuista | it's foggy |
| on pilvistä | it's cloudy |
| on sateista | it's rainy |
| on myrsky | there's a storm |
| on ukkonen | there's thunder |

---

## Vaatteet (Clothing)

| Finnish | English |
|---|---|
| t-paita | t-shirt |
| paita | shirt |
| villapusero | sweater / jumper |
| takki | coat / jacket |
| housut | trousers / pants |
| farkut | jeans |
| hame | skirt |
| mekko | dress |
| puku | suit |
| uimapuku | swimsuit |
| uimahousut | swim trunks |
| sukat | socks |
| shortsit | shorts |
| kengät | shoes |
| saappaat | boots |
| lenkkarit | trainers / sneakers (informal) |
| lenkkitossut | trainers / sneakers |
| alusvaatteet | underwear |
| rintaliivit | bra |
| alushousut | underpants |
| aluspaita | undershirt / vest |
| kalsarit | men's underwear |
| huivi | scarf |
| pipo | beanie / knit hat |
| käsineet | gloves |
| solmio | tie |
| vyö | belt |
| kaulaliina | scarf / neckerchief |
| sateenvarjo | umbrella |

---

## Värit (Colours)

| Finnish | English |
|---|---|
| punainen | red |
| sininen | blue |
| vihreä | green |
| keltainen | yellow |
| oranssi | orange |
| violetti | purple |
| harmaa | grey |
| valkoinen | white |
| ruskea | brown |
| vaaleanpunainen | pink |
| beige | beige |
| turkoosi | turquoise |
| kultainen | golden |
| hopeanvärinen | silver |

---

## Kehonosat (Body parts)

| Finnish | English |
|---|---|
| pää | head |
| kasvot | face |
| silmät | eyes |
| korva | ear |
| nenä | nose |
| suu | mouth |
| hammas | tooth |
| kieli | tongue |
| kaula | neck |
| olkapää | shoulder |
| käsivarsi | arm |
| kyynärpää | elbow |
| ranne | wrist |
| sormi | finger |
| kynsi | fingernail |
| rintakehä | chest |
| selkä | back |
| vatsa | stomach / belly |
| lantio | hip |
| reisi | thigh |
| polvi | knee |
| sääri | shin / lower leg |
| nilkka | ankle |
| varvas | toe |
| sydän | heart |
| keuhkot | lungs |
| aivot | brain |
| iho | skin |
| veri | blood |

---

## Eläimet (Animals)

| Finnish | English |
|---|---|
| koira | dog |
| kissa | cat |
| hevonen | horse |
| lehmä | cow |
| sika | pig |
| kani | rabbit |
| karhu | bear |
| hirvi | moose / elk |
| poro | reindeer |
| kettu | fox |
| susi | wolf |
| orava | squirrel |
| jänis | hare |
| lohi | salmon |
| ahven | perch |
| hauki | pike |
| kotka | eagle |
| varis | crow |
| ankka | duck |
| lintu | bird |

---

## Ruoka ja juoma (Food and drink)

| Finnish | English |
|---|---|
| liha | meat |
| kana | chicken |
| nauta | beef |
| riisi | rice |
| peruna | potato |
| tomaatti | tomato |
| kurkku | cucumber |
| salaatti | salad / lettuce |
| sipuli | onion |
| valkosipuli | garlic |
| paprika | bell pepper |
| porkkana | carrot |
| parsakaali | broccoli |
| juusto | cheese |
| maito | milk |
| kerma | cream |
| jogurtti | yogurt |
| kananmuna | egg |
| voi | butter |
| sokeri | sugar |
| suola | salt |
| pippuri | pepper |
| pasta | pasta |
| keitto | soup |
| pulla | bun / sweet roll |
| kakku | cake |
| mehu | juice |
| vesi | water |
| olut | beer |
| viini | wine |
| tee | tea |
| limsa | fizzy drink / soda |

---

## Tunteet (Emotions)

| Finnish | English |
|---|---|
| surullinen | sad |
| vihainen | angry |
| pelokas | scared / afraid |
| innostunut | excited / enthusiastic |
| hermostunut | nervous |
| ylpeä | proud |
| häpeissään | ashamed |
| rakastunut | in love |
| tylsistynyt | bored |
| yllättynyt | surprised |
| pettynyt | disappointed |
| tyytyväinen | satisfied / content |
| huolestunut | worried |
| helpottunut | relieved |
| mustasukkainen | jealous (romantically) |
| kateellinen | envious |

---

## Paikat kaupungissa (Places in town)

| Finnish | English |
|---|---|
| ravintola | restaurant |
| kahvila | café |
| baari | bar |
| kirjasto | library |
| pankki | bank |
| sairaala | hospital |
| hotelli | hotel |
| museo | museum |
| kirkko | church |
| tori | market square |
| rautatieasema | railway station |
| lentokenttä | airport |
| kauppakeskus | shopping centre |
| teatteri | theatre |
| elokuvateatteri | cinema |
| posti | post office |
| poliisiasema | police station |
| paloasema | fire station |
| stadion | stadium |
| hautausmaa | cemetery |

---

## Liikenne (Transport)

| Finnish | English |
|---|---|
| juna | train |
| taksi | taxi |
| polkupyörä | bicycle |
| laiva | ship / ferry |
| lentokone | aeroplane |
| moottoripyörä | motorbike |
| raitiovaunu | tram |
| ambulanssi | ambulance |
| rekka | lorry / truck |
| silta | bridge |
| pyörätie | cycle path |
| liikennevalo | traffic light |

---

## Koti ja huonekalut (Home and furniture)

| Finnish | English |
|---|---|
| sohva | sofa / couch |
| tuoli | chair |
| nojatuoli | armchair |
| pöytä | table |
| ruokapöytä | dining table |
| sänky | bed |
| patja | mattress |
| tyyny | pillow |
| peitto | duvet / blanket |
| lamppu | lamp |
| ikkuna | window |
| ovi | door |
| hylly | shelf |
| kaappi | cupboard |
| vaatekaappi | wardrobe |
| peili | mirror |
| matto | rug / carpet |
| verho | curtain |
| kylpyamme | bathtub |
| vessa | toilet / bathroom (informal) |
| jääkaappi | fridge |
| pakastin | freezer |
| liesi | stove / cooker |
| uuni | oven |
| pesukone | washing machine |

---

## Vuorokaudenajat (Time of day)

| Finnish | English |
|---|---|
| aamu | morning |
| aamupäivä | late morning / forenoon |
| keskipäivä | midday / noon |
| iltapäivä | afternoon |
| ilta | evening |
| keskiyö | midnight |
| aamunkoitto | dawn |
| hetki | moment |
| tänään | today |
| huomenna | tomorrow |
| eilen | yesterday |
| ylihuomenna | the day after tomorrow |
| toissapäivänä | the day before yesterday |

---

## Mökkisanasto (Cottage vocabulary)

| Finnish | English |
|---|---|
| mökki | cottage / cabin |
| sauna | sauna |
| savusauna | smoke sauna |
| lauteet | sauna benches |
| kiuas | sauna stove |
| löyly | steam (in sauna) |
| vihta | birch whisk |
| avanto | ice hole (for winter swimming) |
| hiilos | embers |
| järvi | lake |
| järvimaisema | lake scenery |
| metsä | forest |
| ranta | shore / beach |
| laituri | dock / pier |
| polku | path / trail |
| nuotio | campfire |
| nuotiopaikka | campfire spot |
| nokipannu | campfire pot |
| polttopuu | firewood |
| savupiippu | chimney |
| kaivo | well |
| vene | boat |
| onki | fishing rod |
| tähtitaivas | starry sky |
| auringonlasku | sunset |
| hyttyset | mosquitoes |
| luonto | nature |
| kellua | to float |
| kastautua | to dip oneself |
| sytyttää | to light / to ignite |
| soutu | rowing |
| grillata | to grill |
| poimia | to pick / to gather |
| kalastaa | to fish |
| meloa | to paddle / to kayak |
| paistaa | to shine / to bake / to fry |
| puolukka | lingonberry |
| mustikka | blueberry / bilberry |
| sieni | mushroom |
| makkara | sausage |
| tikku | stick / skewer |
| kalakeitto | fish soup |
